...
related:
  - constitution
  - plan
  - adr/001-choose-desktop-runtime  # 待创建
---
...
| # | 决策项 | 候选 | 触发 ADR | 阻塞 |
|---|---|---|---|---|
| **D1** | 桌面运行时 | Tauri 2.x / Electron 30+ | ADR-001 | 第一阶段建仓 |
| **D2** | 前端框架 | React 19 / Vue 3.5 / Svelte 5 / Solid | ADR-002 | 第一阶段建仓 |
| **D3** | 数据持久化 | SQLite（drizzle/better-sqlite3）/ JSON 文件 / localStorage | ADR-003 | UsageRecord 实现 |
| **D4** | 包管理器 | pnpm / bun / npm | ADR-004 | 建仓 |
| **D5** | 构建工具 | Vite / Tauri 内置 | 依赖 D1 | 第一阶段 |
| **D6** | 状态管理 | Zustand / Jotai / 框架内置 | 依赖 D2 | 第二阶段 |
| **D7** | 全局快捷键 API | Tauri global-shortcut / Electron globalShortcut | 依赖 D1 | 第一阶段 |
| **D8** | 是否复用 prompt-combiner 旧代码 | 复用 / 重写 / 部分迁移 | ADR-005 | 第一阶段建仓 |

**决策原则**（不是决策本身）：
- 优先选择支持 A1 + A2 + C1 的组合
- Tauri 因更小包体 + 更低内存常驻是默认候选，但需对比 Electron 的成熟度与开发速度
- 若复用 prompt-combiner（[[plan#§总实施周期估计]]），需在 ADR-005 列出迁移成本
