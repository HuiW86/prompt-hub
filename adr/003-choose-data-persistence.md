...
- **日期**：2026-05-19
- **决策者**：omar
- **影响范围**：[[tech-stack#§3-D3]] resolved / [[prd#数据模型]] schema 设计 / [[ops-spec#§3]] 备份策略（影响备份文件是否加密）/ [[ops-spec#§1.1]] 分发包大小（bundled SQLite 增加 ~1MB）
...
### 触发事件
[[prd#数据模型]] 定义了 Modifier / Composition / Macro / UsageRecord / SOP 等 ≥5 个关联实体，UsageRecord 增长后需 query/index，[[adr/001-choose-desktop-runtime]] 锁定 Tauri 2.x（Rust 后端），数据持久化层是 MVP 必决。

### 业务约束
- [[constitution#A2]] 本地优先 + 无服务端 — 排除任何远程数据层
- [[constitution#A3]] 单人单机 — 无多用户隔离需求
- [[spec#8.2]] 话术「含工作指纹」属于隐私敏感数据 — 决定是否需要整库加密的核心判断点
- [[constitution#C1]] 主形态 200ms 唤起 — 数据层启动 + 查询开销不能拖累
...
- **缺点**：
  ...
  - 但 [[constitution#A2]] 已声明「不上云 + 本地优先」是隐私底线，OS 用户登录密码 + FileVault（macOS 默认）已是第一道防线
...
- **缺点**：
  - 需要 master password 解锁流程 — **破坏 [[spec#2.3]] 「召之即来」哲学**（每次启动都要输密码）
...
**为什么不选其他**：
- 不选 Option B（SQLCipher）因为：master password 解锁与 [[spec#2.3]] 召之即来哲学正面冲突，C1 200ms 唤起不可达
...
### 正向后果
- 解 [[tech-stack#§3-D3]]
- [[prd]] 字段级 schema 可直接落地
- [[ops-spec#§3]] 备份策略简化为「cp .db + WAL」
- C1 200ms 唤起的数据层零负担
- 跨设备恢复无需带密钥
- 与 [[spec#2.3]] 召之即来哲学一致
...
- **触发本决策的文档**：[[tech-stack#§3-D3]] / [[prd#数据模型]] / [[spec#8.2]] / [[constitution#A2]] / [[constitution#C1]]
- **被本决策影响的文档**：[[prd]] / [[ops-spec#§3]] / [[ops-spec#§1.1]]
