...
> 项目定位见 [[spec]]、UI 契约见 [[product-spec]]、实施计划见 [[plan]]。
>
...
这一章是对 [[product-spec#4-界面设计原则]] 的具体落地。每个模块会说明：它是什么、为什么存在、关键交互、与其他模块的关系。
...
**与其他模块的关系**：
- **不与 Macro 区互通**——AlignmentPhrase 不出现在 Macro 区，Macro 不出现在相位带（[[spec#8.6-不让对齐层与任务层混淆]]）
...
- 不跳页的展开方式保证"全景不被破坏"（[[product-spec#4.1-布局原则]]）
...
**为什么这样设计**：
- Modifier 按四组排列反映"认知 → 动作 → 产出 → 约束"的思考顺序（哲学五、[[spec#3.1-三层资产模型]]）
...
- **AlignmentPhrase 不进入组合工作台**——协议层与内容层物理分离（[[spec#8.6-不让对齐层与任务层混淆]]）
...
- 长按（触控）/ 右键（鼠标）→ 把该条话术升级为 Macro（如果是任务话术且还不是 Macro 的话）；AlignmentPhrase 类型则不提供"升级为 Macro"选项（[[spec#8.6-不让对齐层与任务层混淆]]）

**为什么这样设计**：
- 回答了使用者原始需求中的"刚刚操作了什么"（哲学二、[[product-spec#4.4-状态反馈原则]]）
...
- 回答了使用者原始需求中的"未来可以怎么操作"（[[product-spec#4.5-引导原则]]）
...
**重要边界**：SOP 步骤**不**引用 AlignmentPhrase（[[spec#8.6-不让对齐层与任务层混淆]]）——相位切换由使用者在执行 SOP 过程中手动通过相位带完成，不混进 SOP 步骤序列。
...
**定位**：承载"界面自身也是被维护的资产"这条原则（[[spec#2.9-哲学九]]）。
...
| group | enum | N | - | 四值枚举，见 [[spec#3.4-分层语言系统]] | 哲学五分层语言四类（cognition / action / delivery / constraint） |
...
- **不带 phase 字段**——Phase 只服务于 AlignmentPhrase，不污染 Modifier（[[spec#3.5-认知相位（Phase）与对齐话术（AlignmentPhrase）]] 决策）
...
- **不带 phase_id 或 is_alignment 字段**——AlignmentPhrase 独立成对象，不寄生在 Macro 里（哲学七、[[spec#3.5-认知相位（Phase）与对齐话术（AlignmentPhrase）]]）
...
|------|------|----------|---------|------------|-------------|
| id | string | N | - | PK | 唯一标识 |
| name | string | N | - | 相位名集合见 [[spec#3.5-认知相位（Phase）与对齐话术（AlignmentPhrase）]]（如 发散/理解/规划/生成/执行/收敛/沉淀/迭代） | 相位名 |
...
- 独立成表（不写死为枚举）——因为 Phase 仍在演化中（[[spec#3.5-认知相位（Phase）与对齐话术（AlignmentPhrase）]]），使用者会根据实践增删
...
参见 [[spec#8.6-不让对齐层与任务层混淆]]。AlignmentPhrase 与其他资产的**禁止性关系**：
...
| SOPStep → AlignmentPhrase | ✗ 禁止 | - | 哲学七边界：SOP 是任务层，AlignmentPhrase 是协议层（[[spec#8.6-不让对齐层与任务层混淆]]） |
...
- **SOPStep 只能引用 Macro 或 Phrase**——不能引用 AlignmentPhrase（[[spec#8.6-不让对齐层与任务层混淆]] 哲学七边界）
...
- `modifier_ids` 让"哪些 Modifier 在实战中被组合得最多"成为可分析数据，是 [[spec#10.1-Modifier-池的增长管理]] 治理的数据基础
- `sop_id` + `sop_step_order` 让 SOP 执行可追溯——可以统计"方案设计 SOP 平均走到第几步就完成了"，是 [[spec#10.2-SOP-的粒度]] 调整的数据基础
...
| N2 | 自动调用任何外部 AI 生成内容 | 任何 LLM API 调用（Claude / OpenAI / Gemini 等） | [[spec#8.1-不做-AI-生成]]：失去自己思考 = 摧毁手动挡核心价值 | 无 override |
| N3 | 自动发送话术给任何 AI | 复制后自动 paste / 自动 send to Claude Code | [[spec#8.3-不做自动执行]]：手动复制粘贴是思考的缓冲 | 无 override |
| N4 | 跨进程 / 跨窗口写入 | 任何写入非应用本身 sandbox 的位置（修改其他应用配置 / 系统目录） | 应用应严格限制在自身数据范围 | 无 override |
| N5 | 持久化 AlignmentPhrase 到 Macro 类型 | 任何 INSERT / UPDATE 将 AlignmentPhrase 内容写入 Macro 表 | [[spec#8.6-不让对齐层与任务层混淆]]：哲学七要求物理分离 | 无 override |
