...
> 全栈技术决议完毕（[[09-tech-stack#§3]]）：Tauri 2.x + React 19.2 + pnpm 10.x + Vite 7.x + Vitest 4 + cargo test。M0 已建仓，本节命令可直接跑（`bench:*` / `lint` / `format` / `test` 脚本第一阶段补齐）。
...
# 性能 benchmark（[[02-constitution#C1]] 200ms 唤起自检，第一阶段实现）
