...
- 下一动作：**M-X.3 UI 收件箱已收口 + 文档涟漪**（features v0.6 / CLAUDE §7）→ commit 本批；M-X 阶段（MCP 反向写入全链路）整体 done。之后并行待办：ADR-012 Phase 5 `pnpm tauri dev` 6 步视觉 diff（overlay frame / ⌥Space 唤起 / PhaseBar 8 段 / AlignmentPhrases chip flash / MacroGrid Flame / StatusBar ⌘K，**新增草稿 tab + 待审 badge + promote/discard 肉眼验收**）+ bench 脚本 + M0-4 签名；P2 defer：`create_draft` 单写 size-cap（M-X.4）；Claude Design pipeline 待 omar 上传 [[CLAUDE-DESIGN]] 跑 Task β；详见 [[HANDOFF#Next-Actions]]
