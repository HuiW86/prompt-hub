...
> ⚠️ **stub — 待第一阶段建仓后回填实际命令**
>
> 运行时已锁定 **Tauri 2.x**（[[adr/001-choose-desktop-runtime]]）；包管理 `<pm>` 待 [[tech-stack#§3-D4]] 决议（候选 pnpm / bun / npm）；测试框架待 ADR-002 决议（候选 Vitest / Playwright）。
...
# 开发与构建
<pm> tauri dev                                            # Dev server + Tauri shell（Vite HMR）
<pm> tauri build                                          # 生产构建（输出 .dmg / .msi / .app.tar.gz）

# 质量
<pm> test                                                 # 单元 + 集成（待选 Vitest / Playwright）
<pm> lint                                                 # ESLint
<pm> format                                               # Prettier
...
<pm> bench:cold-start                                     # 冷启动 P95 测量
<pm> bench:hotkey-wake                                    # 快捷键唤起 P95 测量
```

> 本节是"未来 AI 接手第一阶段"的接力棒——所有 `<pm>` 在 ADR-004 决议后由人统一替换为具体包管理器名。
...
7. **不要在 pre-code 阶段擅自选型**——技术栈未拍板时见 [[tech-stack#§3]]，开 ADR 决议
   - ✅ **D1 已解锁**（[[adr/001-choose-desktop-runtime]] Tauri 2.x，2026-05-19）
   - ⏳ **仍 pending**：D2（前端框架）/ D3（数据持久化）/ D4（包管理）/ D8（prompt-combiner 复用）— 涉及前必须开对应 ADR
...
- ADR 进度：001 Accepted（桌面运行时 = Tauri 2.x）；002~005 pending
- 下一动作：开 ADR-002/003/004/005 → 第一阶段 MVP 建仓（按 [[plan#§0]]）；或见 [[HANDOFF#Next-Actions]]
