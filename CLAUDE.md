...
- 下一动作：**M0-4 签名公证链路已收口**（2026-06-03，M0 四项全绿，runbook [[m0-4-macos-signing]]）+ 前序 ADR-012 Phase 5 验收 11/11 收口。之后并行待办：bench 脚本回归（`bench:cold-start`/`hotkey-wake`）；P2 defer：`create_draft` 单写 size-cap（M-X.4）；Claude Design pipeline 待 omar 上传 [[CLAUDE-DESIGN]] 跑 Task β；详见 [[HANDOFF#Next-Actions]]
