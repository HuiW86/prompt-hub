...
- **最近一轮改动（2026-07-06 Scene 编辑分层化）**：ADR-021 收口——废除 ScenePanel 全局 editMode（1706→949 行），新增 `ScenePropertiesEditor` 属性面板（PRD §6.4 icon/color/rolePresets 首获 UI）+ 子阶段/话术就地动作簇 + ghost 入口；排序拖拽→按钮（能力零回退）；前端 181→222 测试全绿，两轮 verifier 对抗审查 PASS；明细见 CHANGELOG 2026-07-06 条目 + ADR-021 + [[HANDOFF]]
