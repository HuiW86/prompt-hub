...
- ADR 进度：**14 Accepted（015 含补遗-1/-2）+ 1 Proposed + 1 Reserved**——001/002/003/004/006/007/008/009/010 + 012（lock-visual-quality-anchor，Linear 气质锚定）+ 013（alignment-phrases-tab-inclusion，追认独立 region + Tab cycle 6 tab-reachable；谱系备注 2026-06-11 AE P2.4 两编辑区入 cycle 6→8）+ 014（nspanel-isa-swizzle，2026-06-03 Reserved→Accepted，NSPanel 子类 override canBecomeKeyWindow + isa-swizzle 取得 borderless key-window 资格）+ 015（expose-mcp-write-pipeline，2026-05-27 Accepted，14 tool + workspace 拆 4 crate + drafts staging；补遗-1 Modifier/Macro field-mapping 决策 iii / 补遗-2 promote 软删 + serde 约束对齐）；005（prompt-combiner 复用）仍 Proposed；011 Reserved（search UsageSource）
...
- 下一动作：**bench 回归 + M-X.4 size-cap 均已收口**（2026-06-12）——① bench 回归：cold-start P95≈258ms/p50≈175ms、hotkey-wake P95≈13ms，两项无回归（旧 ~0.02ms baseline 证伪，§2 注释已更正）；② M-X.4：64KB cap（PRD §10.1.1）下沉 repo-core 单一出处（`MAX_PAYLOAD_BYTES` + create/update 双检查 + `PayloadTooLarge` + MCP 错误映射），验证全绿（pnpm test 87/87 / cargo test --workspace 51 全过）。剩余唯一待办：Claude Design pipeline 待 omar 上传 [[CLAUDE-DESIGN]] 跑 Task β（外部依赖）；详见 [[HANDOFF#Next-Actions]]
