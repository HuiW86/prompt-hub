...
- 项目阶段：**ADR-012 Phase 1-3 全 ship + Phase 4 设计文档涟漪进行中**（2026-05-25）——Phase 1 fonts/tokens bundle 命名（commit `b932ab4`）/ Phase 2 dark default + light @media + ontology + neutral scale（commit `9a822d8`）/ Phase 3 9 组件按 bundle 视觉重写 + AlignmentPhrases 独立 + tokens precision tier（commit `acf8229`，22 files +695/-421）；Vitest 55/55 + cargo 12/12 + lint + build 911ms 全过；M0-3 实测唤起 P95=10.49ms 远低于 [[02-constitution#C1]] 200ms 死线；M0-4 签名 spike 仍待 Developer ID 证书；Phase 5 `pnpm tauri dev` 6 步 manual verify 待跑
- 文档体系：**13 核心 + L5 协作契约 2 + MANIFEST 全部 ratified**（2026-05-25 design-spec v0.7 / product-spec v0.6 完成 ADR-012 Phase 4 涟漪）——11 篇核心设计文档 `docs/design/01-11-*.md` + L5 派生 [[CLAUDE-DESIGN]] + [[claude-design-prompts]]；全文件清单见 [[MANIFEST]]
- ADR 进度：**12 Accepted + 1 Proposed + 1 Reserved**——001/002/003/004/006/007/008/009/010 + 012（lock-visual-quality-anchor，2026-05-24 Accepted，Linear 整体气质锚定）+ 013（alignment-phrases-tab-inclusion，2026-05-25 Accepted，追认 AlignmentPhrases 独立 region + Tab cycle 6 tab-reachable）；005（prompt-combiner 复用）仍 Proposed；011 Reserved（计划：search UsageSource）
...
- 下一动作：**ADR-012 Phase 5 manual verify + 第一阶段收尾**——design-spec v0.7 / product-spec v0.6 / ADR-013 / features v0.2 / mockup 归档已落盘；等 omar 跑 `pnpm tauri dev` 6 步视觉 diff（overlay frame / ⌥Space 唤起 / PhaseBar 8 段 / AlignmentPhrases chip flash / MacroGrid Flame / StatusBar ⌘K）+ macOS Space fullscreen bug 回归（commit `0fbab2d` 修过）；同步待办：bench 脚本（cold-start / hotkey-wake）/ M0-4 签名；Claude Design pipeline 待 omar 上传 [[CLAUDE-DESIGN]] 创建 design system 跑 Task β；详见 [[HANDOFF#Next-Actions]]
