...
- ✅ 空壳 `.dmg` 通过 notarization，本机 Gatekeeper 放行（M0-4，2026-06-03 实测）——Developer ID Application (NY89JGE95Q) 签名 + hardened runtime + 三项 JIT entitlements（[[m0-4-macos-signing]]）；公证 **Accepted**（私有 API 未被拒，证伪「macos-private-api 与公证冲突」最坏假设）；staple + `spctl` 均 `accepted / Notarized Developer ID`；release 透明窗口运行时正常起、不黑屏。链路 runbook 见 [[m0-4-macos-signing]]
