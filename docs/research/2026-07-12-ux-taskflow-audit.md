...
### 批次 C「排版角色收敛」（发布后，四步）
- C-0：生成"旧计算样式 → 目标角色"**映射表**（逐处比对简写与 preset 的 size/weight/lh/tracking/color 五轴，标记完全等价/有偏差）。⚠️ 批次 A verifier 记账：`Toast.module.css:41 .action` 为批次 A 新增的第 64 处 `font:` 简写债，C-0 必须纳入
