...
>
> **P1-b 代码已实施 2026-08-20**（`4535efa` + `8aa86f8`）：迁移面**实为 4 个不是 5 个**（`RecentList` 无编辑器，属 P2 键盘层，清单误收）；Macro / 草稿两处手搓表单删除收编进 `PhraseFormEditor`；Scene 属性按 omar 裁决走「锚定但点外拒绝关闭」。同时修掉 P1-a 容器的两处既存缺陷（超高面板footer 出界 / 焦点后代卸载后 Esc 失聪）。**373 测试全绿，但 G1 项 5 与项 2 的 B 半仍未验**——布局与视觉 jsdom 验不了，真机走查前不算 landed。明细见 [[CHANGELOG]] 2026-08-20。P2–P3 待排。
...
**P1-b 门（2026-08-20 起未清）**——代码已落、jsdom 全绿，但两项真机验收未做，**清前不算 landed、不入对外发布说明**：

| 项 | 内容 | 为什么 jsdom 验不了 |
|---|---|---|
| G1 项 5 | 在 hover-lift 的 Macro / Scene 卡片上开浮层，hover 进出时位置不跳 | 卡片 hover 施加 `transform`，验的是 top layer 是否真的逃出该包含块——jsdom 无布局引擎 |
| G1 项 2 的 B 半 | Scene / 草稿列**纵向**滚动时浮层跟随不脱锚 | A 半（订阅逻辑）轴无关，P1-a 已补 11 条测试覆盖；B 半是像素问题，且 P1-a 只验过横轴，**不能拿横轴结论顶纵轴** |
