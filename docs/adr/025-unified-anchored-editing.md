...
>
> **G1 走查已于 2026-08-19 执行**：项 1/3/4 omar 目视通过、项 6 实测通过、项 2/5 deferred 到 P1-b 门（明细与理由见 §6 走查结果表）。**omar 拍板 P1-b 可开工**，代价是滚动跟随那条链路带着零覆盖进入迁移。
>
> **P1-b 代码已实施 2026-08-20**（`4535efa` + `8aa86f8`）：迁移面**实为 4 个不是 5 个**（`RecentList` 无编辑器，属 P2 键盘层，清单误收）；Macro / 草稿两处手搓表单删除收编进 `PhraseFormEditor`；Scene 属性按 omar 裁决走「锚定但点外拒绝关闭」。同时修掉 P1-a 容器的两处既存缺陷（超高面板footer 出界 / 焦点后代卸载后 Esc 失聪）。373 测试全绿。**G1 项 5 与项 2 的 B 半已于当日真机走查通过**（详见 §6 走查结果表，首次取得 AI 观测证据），P1-b 至此收口。明细见 [[CHANGELOG]] 2026-08-20。P2–P3 待排。
...
| 2 | **通过**（当日补验，见下） | 拆两半分别取证 |
| 5 | **deferred → P1-b 门** | **P1-a 阶段不适用**：本项要求在 hover-lift 的 Macro / Scene 卡片上开浮层，而 P1-a 只接对齐话术一处，那几个编辑面仍是流内 `EditorPanel`，无浮层可开。G1 原文把一个只有迁移后才存在的对象写进了迁移前的门。**2026-08-20 P1-b 代码落地后该对象已存在，本项仍未验**——见下 |

**P1-b 门走查结果（2026-08-20，dev 实例真机）**——两项**均通过**，且**首次取得 AI 侧观测证据**（P1-a 三轮共 135 帧屏幕捕获从未拍到浮层；本轮改按窗口 ID 定向、按需截图后一次拍中）：

| 项 | 结论 | 证据 |
|---|---|---|
| G1 项 5 | **通过** | Macro 卡「借力最优解」上开浮层，鼠标移入 / 移出各截一帧：**浮层区域逐像素零差异**（diff bbox `None`，最大通道差 `0`），同帧卡片区域最大通道差 `231`。卡片确实在 hover-lift，浮层纹丝不动——top layer 真的逃出了 `transform` 包含块 |
| G1 项 2 的 B 半 | **通过（带一处 1pt 已知量）** | Scene 话术卡上开浮层（**向上翻转**，下方空间不足），纵向滚动容器滚动后模板匹配测位移：锚点侧内容 `-168px`、浮层 `-166px`（物理像素，@2x）。反向滚回原位、连滚多次，**差值恒为 2 物理像素 = 1 逻辑点，不累积** |

**那 1 逻辑点**：已 A/B 排除是本轮新增的高度上限所致——同一滚动位置、同一面板，`maxHeight`/`overflowY` 开与关两版**浮层位置完全一致**（模板匹配 `+0px`，误差 `0.00`）。成因未归因，疑为翻转态 `top = a.top − OFFSET − p.height` 中 rect 分数值的取整差。**记为已知量，不阻塞**：亚感知、恒定、不累积、反向滚动可复现。若日后要归因，第一嫌疑是首次 `useLayoutEffect` 放置与后续重算之间 `p.height` 的取整差别。

走查同时复现两项既有记录：浮层在 Scene 卡上正确逃出 `.sceneCard` 的 `overflow: hidden`（压在 Macro 区之上绘制）；hover 动作簇仍遮挡话术卡标题（`ScenePanel.module.css:420`，随 P2 子决策 3.3 解决）。

**走查方法**（可复用，与 P1-a 的横轴走查同源）：窗口 ID 定向 `screencapture -l<id>`（**禁止全屏**）+ CGEvent 合成鼠标移动/点击/滚轮 + PIL 模板匹配测位移。全程只点铅笔不点卡片本体（卡片本体是复制热区，会触发 hide-on-copy），事后核对状态栏「今日复制 0 次」**未产生任何写入**，无需造临时数据、无需回滚。

**项 2 的补验（2026-08-19 当日，omar 指示先补再迁）**——原判"当前数据下物理不可验"成立，但那只挡住了真机那一半，逻辑那一半一直可测却一直没测。故拆开分别取证：

| 半 | 内容 | 证据 |
|---|---|---|
| A · 订阅逻辑 | 订了哪些祖先、滚动/resize 是否触发重算、卸载是否解绑 | `useAnchoredPosition.test.ts` **新增 5 条**（6→11）。**逐条反向验证**：把 hook 分别改成「只订最近祖先」/「不过滤 overflow」/「泄漏 scroll 监听」，三种破坏各自**只**被对应一条测出 |
| B · 真机跟随 | 浮层是否随 chip 行横滚跟手、不脱锚 | **omar 真机确认**。造 14 条临时话术撑溢出 chip 行后验证，验毕按 `id LIKE '__g1tmp__%'` 精确删除并与操作前整库备份逐字段比对（原 13 条零差异）。AI 侧 60 帧捕获仍未拍到浮层，故此半亦不含 AI 观测证据 |

> 遗留认知：A 半摘掉了 `scrollableAncestors` 的 jsdom 零覆盖；B 半的真机证据是人工目视，不可回归。P1-b 迁移 Scene / Recent 两个纵向滚动容器时，跟随行为会在**纵轴**上首次被真实使用——A 半的测试是轴无关的，B 半则需在 P1-b 门重新目视一次。

**P1-b · 容器迁移**：其余编辑面跟迁。迁移前先定 `AnchoredEditor` 与 `PhraseFormEditor` 的接口契约（`layer` / `extraFields` / `submitLabel` / 自定义 className 全覆盖），否则迁移工时不可信——该项已按 omar 裁决**提前到 P1-a** 完成。

**P1-b 清单纠偏（2026-08-20）**——本 ADR 此前多处写「其余五个编辑面」，逐文件核实后**只有 4 个**：

| 编辑面 | 迁移前 | 迁移后 |
|---|---|---|
| Scene 话术 `scene/PhraseEditor.tsx` | `PhraseFormEditor presentation="inline"` | `anchored`，锚点为话术卡 / 列尾 ghost 按钮 |
| Macro `MacroGrid.tsx` | 裸 `EditorPanel` + **手搓表单** | 手搓表单删除，收编进 `PhraseFormEditor` |
| 草稿 `DraftInbox.tsx` | 裸 `EditorPanel` + **手搓表单** | 同上，锚点为该卡的「编辑」按钮 |
| Scene 属性 `ScenePropertiesEditor.tsx` | 裸 `EditorPanel` | 裹 `AnchoredEditor`，**点外拒绝关闭**（见下） |

`RecentList.tsx` **没有编辑器**——它是只读复制列表。§1 影响范围列它是因为它属 P2 键盘动作层与 hover-lift 五处之一，被迁移清单误收。这与 G1 项 5 是同一类错误：**清单引用了一个在本阶段不存在的对象**。

**Scene 属性的例外（omar 2026-08-20 裁决）**：其余三处照子决策 2 规则表「点外 = 保存并关闭」，Scene 属性**不**照办，`onDismiss("outside")` 恒返回 `false`。理由是它不是一次性属性微调而是有界任务（改名 + 换色 + 改角色预设 + 内嵌永久删除确认），HIG 里这形状是 sheet 不是 popover；两个具体风险是删除确认框展开时点外会连确认框一起吞掉，以及顺手点中的颜色会在注意力离开的瞬间变成一次落库写入。代价是点外行为与另三处不一致，故记在此。Esc 相应改为**逐层退栈**：半截角色预设 → 删除确认 → 面板。

**迁移中发现的两处 P1-a 既存缺陷**（已在同批修复，非新决策）：

1. **超高面板的 footer 出界**——只夹取**原点**不够：高于 frame 的面板被钉在顶部内缩处，footer（保存 / 取消）跑出画面下沿，正是本 ADR 触发事件 A 的缺陷在另一块面板上复现。`useAnchoredPosition` 增报 frame 相对的 `maxHeight` + 面板内部滚动。该值**不能写进 CSS**：外壳是 inset 浮动框而非视口。
2. **焦点后代卸载后 Esc 失聪**——Esc 守卫是「面板当前含 activeElement」。内联确认框被自己的取消键收起时，被聚焦的节点随之卸载，焦点落到 `<body>` 且不触发 `focusin`，面板从此对 Esc 永久无响应，直到用户重新点回面板。现补：焦点从本面板掉落且无处着落时，本面板仍持有该键。
