...
- **日期**：2026-05-19
- **决策者**：omar
- **影响范围**：[[05-design-spec]] token → CSS variables 直接映射 / [[CLAUDE#§4.1]] 禁止裸值铁律可机械检测 / 第一阶段所有组件实现 / 第二阶段（[[prompt-hub-mvp#第二阶段]]）触发重评估
...
### 触发事件
[[002-choose-frontend-framework]] 锁定 React 19.2 后，组件样式方案是 MVP 第一个组件落地前必决项。

### 业务约束
- [[CLAUDE#§4.1]] CSS 必须用 token — 任何组件 CSS / 内联样式禁止裸 px / 裸 hex / 裸 ms 值
- [[05-design-spec]] v0.6 已定义完整 token 体系（CSS variables 形式）+ [[prompt-hub-mvp#§0-T1]] 已落根样式表
- 历史事件：[[prompt-hub-mvp#§0-T2]] `#1D9E75` 字面量混入代码导致颜色不一致（2026-05-18 全量替换）— 「禁止裸值」是事故沉淀的规则

### 技术约束
- 与 React 19.2 + Vite 8.0 集成
- 与 [[002-choose-frontend-framework]] 选定的 lucide-react / framer-motion（按需）兼容
...
- **优点**：
  ...
  - 与 [[05-design-spec]] token 直接映射（无中间层）
...
- **描述**：引入 Tailwind 4，通过 preset 将 utility class 全部映射到 [[05-design-spec]] token
- **优点**：
  - utility class 开发速度快
  - 生态丰富（社区组件库 / 文档 / 示例）
  - VaultX 实战验证
...
> **一句话拍板**：选择 **纯 CSS variables + CSS Modules**，理由是与 [[CLAUDE#§4.1]] 「禁止裸值」铁律结构性契合，铁律可机械检测，token 直接映射零中间层；第二阶段（[[prompt-hub-mvp#第二阶段]] 组件复杂度上升后）重评估 Tailwind token-only 模式。
...
### 正向后果
- 解 [[09-tech-stack#§3]] 样式方案悬置
- [[CLAUDE#§4.1]] 铁律可用简单 lint 规则机械检测（`/[\d.]+px|#[0-9a-fA-F]{3,6}|\d+ms/` 在 `.module.css` 中报警）
- [[05-design-spec]] token 直接映射 CSS variables，无 preset 中间层
...
- **触发本决策的文档**：[[CLAUDE#§4.1]] / [[05-design-spec]] / [[prompt-hub-mvp#§0-T1]]
