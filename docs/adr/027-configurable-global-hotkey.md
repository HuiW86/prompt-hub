...
>
> 因此本 ADR §5 子决策 2 的「冲突只有 `register()` 失败一种形态」**对探测是对的，对用户体验是不完整的**：`HotkeyUnavailable` 仍必要（竞态、系统保留但非 Carbon 持有的组合键会走到它），但冲突的**主要形态**是「按下去本窗口没反应、另一个应用跳出来」，界面不给任何解释。
>
> ✅ **omar 当日拍板补提示，已落地并复测通过**。判据不用计时器（会误伤犹豫的用户），用「**修饰键按下又抬起、期间没收到任何主键**」——修饰键本身是普通按键会正常送达，被截走的只有主键。提示后保持录键态。**这不新开 ADR**：单选项、可逆、不改本 ADR 任何已批子决策（按 [[CLAUDE#§5]] §5.3 不构成不可逆的多选一）；契约落 [[03-product-spec]] **v0.21** §13.3 区域 9，测试落 [[11-test-spec]] **v0.5**。
>
> **G3 项 2 随之由「不可达」转为「通过」**：第二个实例占住 `⌥Space`，在第一个实例录键态按 `⌥Space` → 提示按预期出现、录键态保持、绑定未变、旧组合键事后正常。**这个门项此前测不了，恰恰是因为产品缺了这条提示**——门项不可达本身就是缺陷的症状，不只是门项写得不好。
