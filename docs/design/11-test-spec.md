...
| **G3（四项）**| ADR-027 全局唤起键可配置 | **3 通过 / 1 不可达**（2026-08-20 走查，见 §4.2）。项 2 不是失败——**是门项本身写错了**，那个场景在真机上根本按不出来。**omar 当日另行真机走查，未发现问题**（人工目视，不可回归；覆盖到哪几项未逐条记录）|
...
| 2 | ⚠️ **不可达** —— 见下方「门项写错了」 | 录键态下按被占组合键，**录键器根本收不到**：事件被持有方在 OS 层消费。A 的绑定保持 `Control+Shift+KeyP` 不变、`⌃⇧P` 事后仍正常 |
| 3 | ✅ **通过（两次复现）** | 窗口隐藏 → 点 Dock 图标 → 窗口上屏。**同时证实「点 Dock 图标无反应」的旧缺陷已修** |
| 4 | ✅ **通过** | `kill` 后重启，启动无 stderr 告警（注册成功），`⌥Space` 无反应 / `⌃⇧P` 唤起——绑定确实读自 SQLite |

> ⚠️ **项 2 门项写错了，不是实现失败**（本轮最重要的产出）。原门项设想「按下一个已被占用的组合键 → 报『已被其他应用占用』」。真机上这个场景**按不出来**：被占用的组合键由持有方在 OS 层注册并**消费**，前台应用的 webview 根本收不到该按键。实测用第二个实例占住 `⌥Space`，在第一个实例的录键态按 `⌥Space` —— 结果是**第二个实例的窗口弹了出来**，第一个实例的录键器全程没有任何输入。
>
