...
**布局与视觉权重 jsdom 验不了，以下两项清掉之前不算 landed、不入对外发布说明**：

- **G1 项 5**：在 hover-lift 的 Macro / Scene 卡片上开浮层，hover 进出时位置不跳——验的是 top layer 是否真的逃出 `transform` 包含块
- **G1 项 2 的 B 半**：Scene / 草稿列**纵向**滚动时浮层跟随不脱锚。A 半（订阅逻辑）轴无关、P1-a 已有 11 条测试覆盖；B 半是像素问题，且 P1-a 只目视过横轴，**不能拿横轴结论顶纵轴**
