...
**v0.2.0 已 publish**（tag `92fce2a`，run `32422441378`）。版号取 minor 而非 patch：这批带**不可逆的数据库迁移**（`user_version` 11→12），版号该把「装上回不去」说出来；另有三处用户可察觉的改动——快捷键设置页、键盘循环 6 站→5 站、两枚层标签消失。

发布前 runbook §1 / §1.5 / §3 全过。其中第 ③ 项特别值得记：`stapler validate` worked、`spctl` 报 `source=Notarized Developer ID` —— **这正是 v0.1.0 栽过的地方**（当时 `codesign` 全绿而公证被静默跳过，CI 照样绿）。这次公证是真的做了，CI 里的 `Assert notarized + stapled` 也逐架构通过。

### [[017-enable-auto-update]] Phase 6 销账：真实更新链路首次跑通

这一项挂了两个月，**不是没人做，是在真发布过一次之前物理上做不了**：draft 不被 GitHub 当作 `releases/latest`，端点始终 404，没有「上一版客户端」可以去发现新版本。v0.2.0 是第一个可被上一版发现的更新。

实测（已安装 `0.1.1` → `0.2.0`）：设置 › 更新 →「检查更新」→ **「发现新版本 0.2.0」** →「下载并安装」→ 进程重启（PID `80811`→`81316`）→ 安装目录版本变为 `0.2.0`。

**更新后又验了一遍签名链**：`codesign --verify --strict` 通过 / `stapler validate` worked / `spctl` accepted。这一步不是多余的——更新是把新包解包替换进 `/Applications`，若这一步弄坏签名或丢了公证票据，**Gatekeeper 要到下一次冷启动才发作**，届时很难回溯到更新那一刻。

### 顺带：新布局的视觉验证有了

我此前说层标签删除与 Tab 变更「jsdom 验不了、真机走查前不入发布说明」。更新后的 0.2.0 截图正是该验证：协议带顶部与任务列顶部**没有留下孤立空白**，band 直接接相位行、任务列直接接 Macro 区头。**发布本身反倒补上了那次没做的走查。**

### runbook §3 两条已修（发布前赶上了）

- 第 1 项「与本地构建产物逐项比对」→ 重写为四项**做得下去**的核对（版本 / 签发者 / 架构 / 文件清单）。旧表述按字面执行必然失败——本地与 CI 不可能逐字节相同——**必然失败的检查等于没有检查**
- 第 4 项补 **Ed25519 内联验签脚本**，只依赖 `cryptography`，不需 `brew install minisign`（为验一次签名装个工具，实际后果是这步被跳过）。已对 v0.1.1 真实产物跑通并做了改一字节的负例；本次对 v0.2.0 两个架构均 `OK`

---

## 2026-08-20（四）· 第六段 — ADR-026 两项遗留裁决：层标记 pill 减二留一 + SOP 退出 Tab cycle

### 变更内容

[[026-fixed-spatial-layout]] 当初显式不裁、留给 design-spec 回流的两项，omar 本日裁决并落地。

| 文档 | bump | 改动 |
|---|---|---|
| [[03-product-spec]] | v0.21 → **v0.22** | §13.4 Tab 行 6→**5**；§13.3 区域 6 新增「目标形态 ≠ 当前实现」告示块 |
| [[05-design-spec]] | v0.18 → **v0.19**（与密度层同版本） | §5 反设计清单新增「不用文字标签解释界面自身的层级架构」 |
| [[07-features]] | v1.16 → **v1.17** | §3.6 层分离行补记；§6 变更日志 |

### ① 三枚 pill，删两枚留一枚

界面上原有三枚层标记文字标签。**它们看起来是一组对称装饰，其实其中一枚承重**：

| pill | 所在 | 裁决 | 理由 |
|---|---|---|---|
| 「协议层」| `ProtocolBand` | **删** | 它就贴在暗色 band 上，band 本身已经说明了层级 |
| 「任务层」| 任务列顶部 | **删** | 列的位置已经说明了层级 |
| 「协议层 · 参考」| `ModifierGrid` | **留** | 该面在 aside 列，**没有 band、也没有任务列的位置线索**，这枚 pill 是它唯一的层级标识；而它偏偏坐在任务侧的列里，最容易被误读为任务层资产 |

前两枚属**解释性 UI**——界面在向用户讲解自己的架构，而 [[05-design-spec]] §2.4.1 / §5 早就定了层级靠**位置 + 形状**冗余编码。删掉它们不是减配，是回到既定契约。

> **这与 [[026-fixed-spatial-layout]] 裁决表第 3 项是同一条纪律**：那次是「`.phase.active::after` 外观像装饰、实为承重件，必须显式标注」；这次是「三枚 pill 外观像一组，其中一枚承重，不能一起删」。**对称的外观不等于对称的职责。**

### ② SOP 退出 Tab cycle：在屏 ≠ 值一个键盘停靠点

`SopProgress` 至今是单行占位（一个标题 + 一句「第三阶段实现」）。此前它既在屏、又占着区域级 Tab 的一站，于是**键盘用户每轮循环都要在那句占位文字上白停一次**。

裁决：**保留显示，退出 Tab**。区域级 Tab 由 **6 站降为 5 站**。

- 哲学二「同屏可见」只要求**在屏**——这一条继续满足
- 「值一个键盘停靠点」是**另一个**主张，今天不成立
- `data-region="sop-progress"` 保留，它仍是编号区域；真正的 SOP 导航器落地时 `tabIndex` 随它一起回来

顺带把 §13.3 区域 6 的长期账实不符**显式标注**了：正文描述的仪表卡是第三阶段目标形态，当前实现是单行占位。此前两者混在一起读不出区别。

### 测试

`App.test.tsx` 的区域 tabindex 断言由 6 改 5，并**显式钉住「在屏但不可 Tab」**——把「可见」与「可 Tab」拆成两条断言，将来恢复停靠点必须是个有意动作，不能靠改一个数字悄悄发生。398 测试全绿。

---

## 2026-08-20（四）· 第五段 — 密度层：立论重写并首次收录（omar 裁「留」）

### 变更内容

旧账复核：紧凑档写下的唯一存在理由是「640px 基准窗口的高度大半被结构 chrome 吃掉」，而**该基准不存在**——主形态窗口每次唤起都重新铺满当前显示器，`tauri.conf.json` 的 `800×600` 只是占位。omar 裁定**档位保留**，理由重写。

| 文档 | bump | 改动 |
|---|---|---|
| [[05-design-spec]] | v0.18 → **v0.19** | **新增 §2.7 密度层**——立论 + 四条契约 + 收益来源 |
| `src/styles/tokens.css` §3c | — | 注释块重写（同一个不存在的基准） |
| `src/stores/settingsStore.ts` | — | `Density` 类型注释同步 |

### 这是第三个「落地了但 design-spec 从没记过」的契约

紧凑档自 v0.13 起就在 `tokens.css` 落地、由 `density-gate.test.ts` 守着，**design-spec 通篇没提过它**——与 v0.18 补的 z 标尺、v0.4 补的两个源码级 gate 是同一类空白。

**缺席的后果不是没人用，是没人能对它提出异议。** 一个只在代码注释里、且理由还是错的档位，既不会被复核也不会被推翻——它只会一直在。

### 新立论（可复现）

- **变量不是窗口，是显示器**：窗口恒等于屏高，约束来自用户可能用的**最小屏**
- **结构 chrome 近似恒定**（任务区四周约 `263px`，2026-08-19 实测），屏越小占比越大
- **1366×768 上**：任务区约 `505px` vs 两条硬下限合计 `421px` —— 余量 84px，两条常驻横幅同时出现时**只剩约 18px**
- 紧凑档买回的正是这段余量

### 收益标明了来源

任务区外框竖向省 `20px`（相位带 8 + 对齐话术 8 + 状态栏 4），其内每个区域头再省 `8px`，主要收益来自重复行单元（Macro 磁贴 `56→44` / Scene 行 `32→28`）。**这些数字由 token 数值计算得出，不是屏幕实测**——已在正文标注，免得下一个人把它当实测引用。

---

## 2026-08-20（四）· 第四段 — 改键冲突的第二种形态：按键被截走时给提示

### 变更内容

第三段的走查发现「冲突时界面毫无解释」，当日 omar 拍板补提示并落地复测。

**这不是新功能，是把一个只做了一半的交代补齐**：ADR-027 子决策 2 写的「冲突只有 `register()` 失败一种形态」，对**探测**是对的，对**用户体验**是不完整的——真机上更常见的那种冲突，`register()` 根本没机会被调用。

| 文档 | bump | 改动 |
|---|---|---|
| [[03-product-spec]] | v0.20 → **v0.21** | §13.3 区域 9 快捷键页：冲突由「一种形态」改写为**两种形态**，各自的界面交代写进契约 |
| [[07-features]] | v1.15 → **v1.16** | §3.4 该行补冲突提示与测试覆盖；§6 变更日志 |
| [[11-test-spec]] | v0.4 → **v0.5** | 前端 395→**398**；**G3 项 2 由「不可达」重写为「通过」** |

### 两种冲突形态

1. **注册被拒**（竞态、系统保留但非 Carbon 持有的组合键）→ `register()` 失败 → 报「已被其他应用占用」+ 回滚旧绑定。**原本就有**
2. **按键根本没到**（更常见）→ 占用方在系统层截走，本应用既拿不到按键也拿不到错误 → 用户看到「按下去什么都没发生，而另一个应用跳了出来」。**本次补的是这一种**

### 判据为什么不是计时器

计时器会误伤犹豫的用户——盯着键盘想三秒就被告知「可能被占用」。改用**修饰键按下又抬起、期间没收到任何主键**：修饰键本身是普通按键会正常送达，被系统截走的只有主键，所以这个「影子」正是被吞掉那次按键的签名。提示出现后**保持录键态**，用户直接换一组即可。

### 门项不可达本身就是缺陷的症状

G3 项 2 上一段被判「不可达」，当时的结论是「门项写错了」。补完提示再看，更准确的说法是：**那个场景之所以观测不到，正因为产品在那里什么都不显示**。门项测不了和用户看不懂，是同一件事的两面。

### 复测时又踩了一次「怀疑复现手段」

第一次复测**提示没出现**，差点结论成「修复无效」。实为走查工具不忠实——`key.swift` 只在主键上打修饰键标志位，**从不发送修饰键自身的按下/抬起事件**，而判据恰好依赖那对事件。人手按 `⌥Space` 会发，合成的不会。改用 `chord.swift`（先发修饰键 keydown → 主键 → 反序抬起）后一次复现成功。**这是第三段「拍不到 ≠ 不存在」的同一条教训换了个面孔：没复现要先怀疑复现手段。**

### 验证

前端 **398/39 文件**、eslint / tsc / prettier / `pnpm build` 全绿。真机复测：第二个实例占住 `⌥Space` → 第一个实例录键态按 `⌥Space` → **提示按预期出现**、录键态保持、绑定仍为 `Control+Shift+KeyP`、旧组合键事后正常。走查跑在隔离 `HOME` 上，**真实资产库未触碰**（已核对绑定值未变）；⚠️ 但界面偏好不受 `HOME` 隔离，见下。

### 撤销一条自报的疑点：「首装是浅色」不成立

走查时用隔离 `HOME` 起实例，界面是浅色、设置里「浅色」为选中态，一度记为「首装可能拿不到 [[024-dark-cockpit-identity]] 的深色身份」。挖出那份存档：`{"themeMode":"light","accent":"blue","density":"compact"}` —— **是 omar 自己的旧开发版偏好，不是首装默认值**，疑点撤销。

根因值得记档：**`HOME` 覆盖只隔离数据库，隔离不了界面偏好**。SQLite 确实落到了临时目录，但 WebKit 的 localStorage 不认 `HOME` 覆盖，照旧读写真实的 `~/Library/WebKit/prompt-hub/...`。于是「空白配置」这一半根本不空白。

⚠️ **连带更正一句说过头的话**：第四段原写走查「全程隔离 `HOME`，真实资产库未触碰」——**资产库确实没碰（已核对绑定值未变）**，但界面偏好被写到了真实存储里。半隔离不该被描述成隔离。

### 为什么不开 ADR

单选项、可逆、不改 ADR-027 任何已批子决策——按 [[CLAUDE#§5]] §5.3 不构成「二选一的不可逆决策」。裁决与证据记在 [[027-configurable-global-hotkey]] §2。

---

## 2026-08-20（四）· 第三段 — ADR-027 全局唤起键可配置（决策 + 落地 + 回流同日闭环）

### 变更内容

[[027-configurable-global-hotkey]] 起草 → omar 拍板 → 当日落地 → 契约回流，同日走完一整圈。

**这不是新功能，是销账**：[[03-product-spec]] §13.4「`⌥ Space` 默认值，**可配置**」自 v0.5 起在册约三个月，实现一直硬编码于 `src-tauri/src/lib.rs:165`。它是第二段回流时查出的三处账实不符之一（另两处当场处置完毕），也是唯一需要真实工作量的一处。

| 文档 | bump | 改动 |
|---|---|---|
| [[03-product-spec]] | v0.19 → **v0.20** | §13.3 区域 9 新增**「快捷键页」**（设置弹窗 3 页 → 4 页）+ 新增**「设置持久化归属」表**；§13.4 `⌥ Space` 行补改绑入口与存储位置、新增 `ESC`（录键态）行、`⌘,` 行页签枚举 3→4 |
| [[06-prd]] | v0.12 → **v0.13** | §5.8 可配置项**补「全局唤起键」**（本清单此前唯一的缺口）；**新增 §6.8-bis Setting** —— `settings(key PK, value)`，刻意排除在 §6.0 资产总览之外 |
| [[07-features]] | v1.14 → **v1.15** | §3.4 新增「全局唤起键可配置」→ `done`；§4 节奏表 S4 行 4→**5** 功能，合计 87 → **88**；§3.6 全局快捷键注册行补记绑定来源变更 |
| [[11-test-spec]] | v0.3 → **v0.4** | 前端 373→**395** / Rust 158→**168** / IPC 51→**53**；新增 **§4.2 G3 门（四项）** |

### 三条子决策

1. **绑定存 SQLite `settings` 表，不是 localStorage**。理由只有一条且是硬的：快捷键在 Tauri `setup()` 里注册，那一刻 webview 还不存在。localStorage 方案只能「先注册默认键、前端挂载后再改」，而**改键的用户恰恰就是默认键冲突的那批人**——等于每次启动必然先错误注册一次。`open_and_migrate` 本就排在 `register()` 之前，SQLite 方案时序上零改造
2. **冲突探测只有 `register()` 失败一种形态**（macOS 无 API 枚举占用者）。改绑走「校验 → 注销旧 → 注册新 → 落库」，**顺序是决策不是实现细节**：先注册成功再落库，保证「库里的值 == 当前真正生效的值」；任一步失败回滚旧绑定。硬约束：强制至少一个修饰键——裸键全局快捷键会在所有应用里吞掉该键，而唯一的撤销途径正是这个键本该唤起的窗口
3. **新增 macOS `RunEvent::Reopen` 逃生口**，防「配置把自己锁在门外」。**顺带修掉一个独立缺陷**：窗口 `visible: false`，此前点 Dock 图标毫无反应

### 起草时更正了 HANDOFF 自己的两处记载

- **「破 `settingsStore.ts:18` 的界」不成立**：该注释主语是 *appearance prefs*，唤起键是行为绑定不是外观偏好，**边界从一开始就没盖住它**。真正要新写的是一条可机械执行的归属判据（**何时被读**，而非重要程度）
- **`settingsStore.globalHotkey` 是死字段**：有默认值、有 setter、不在 `partialize` 白名单、全仓零消费者，而两条测试正钉着一个没有读者的机制。副产品：它的默认值 `"Alt+Space"` 恰好就是插件的 accelerator 格式，格式选型无需重拍

### 实施期两处口径修订（写进 ADR §2，不改原文）

1. **缺行回落默认值，不再 `fail_startup`**：原文写「读不到 = 库损坏」，与子决策 3 的第三层逃生口（`sqlite3` 手改 `settings`）自相矛盾——手改时打成 `DELETE` 会让应用**永久开不了机**，把逃生口变成砖头
2. **`set_global_hotkey` 必须是 `async` 命令**：插件的 `register()` 内部 `run_main_thread!` 派发并阻塞等结果，而同步 IPC 命令本身就跑在主线程上 —— 同步版本首次调用即主线程自锁死

### 验证

前端 **395/39 文件**、Rust **168**、eslint / tsc / prettier / clippy / fmt / `pnpm build` 全绿；`bench:hotkey-wake` p95 **13.708ms**（C1 200ms 预算内，与签名后基线 12.9–14.2ms 持平，无回归）。

**+22 前端用例的逐文件构成经 worktree 对拍取得**，其中 `token-gate` 39→40 **是它自己长出来的**——该 gate 按 CSS module 文件枚举，新增的 `HotkeyRecorder.module.css` 自动入册并通过。⚠️ 手数 `it(` 会漏：多个文件用 `it.each` 生成用例，源码里的 `it(` 数与运行时用例数不等，本轮首次改用 JSON reporter 对拍才发现那 +1。

### G3 真机走查（2026-08-20 当日补跑）：3 通过 / 1 不可达

隔离 `HOME` 跑 dev 二进制 + vite，全程未触碰真实资产库。

- ✅ **项 1**：录键 `⌃⇧P` → DB 由 `Alt+Space` 改为 `Control+Shift+KeyP`，keycap 同步、「恢复默认」由禁用转可用；此后 `⌥Space` 连按两次窗口均不出现，`⌃⇧P` 正常 toggle
- ✅ **项 3**（两次复现）：窗口隐藏 → 点 Dock 图标 → 上屏。**「点 Dock 图标无反应」的旧缺陷确认已修**
- ✅ **项 4**：`kill` 后重启，启动无告警，旧键死、新键活——绑定确实读自 SQLite
- ⚠️ **项 2 不可达 —— 门项写错了，不是实现失败**

> **本轮最重要的产出**：项 2 设想「按下已被占用的组合键 → 报『已被其他应用占用』」，真机上**按不出来**。被占用的组合键由持有方在 OS 层注册并**消费**，本应用的 webview 收不到该按键。实测用第二个实例占住 `⌥Space`，在第一个实例的录键态按 `⌥Space`——结果是**第二个实例的窗口弹了出来**，录键器全程零输入，绑定与旧键均未受损。
>
> 所以 `HotkeyUnavailable` 路径仍必要且正确（竞态、系统保留但非 Carbon 持有的组合键会走到它），但它**不是冲突的主要形态**。主要形态是「按下去本窗口没反应、另一个应用跳出来」，界面不给任何解释——**这是新发现的 UX 缺口，不在 ADR-027 已批范围内**，已挂 [[HANDOFF]] 待裁。
>
> **验收门方法论又补一条**：§4.1 教训 1 说「门项必须在本阶段跑得起来」，本轮四项的**对象**确实都存在，但项 2 的**前提假设**（这个按键能到达应用）从没被检验。起草门项要多问一句：**这个操作真的做得出来吗，不只是这个对象存在吗。**

### 走查工具链踩到的三个坑（记档以免重犯）

1. **debug 裸二进制走 `devUrl`，不跑 vite 就是一个空窗口**。表现为截图全黑 + 键盘鼠标全部失效，极易误判成「窗口渲染不出来」或「权限没给」。控制组（抓 iTerm 窗口正常）是分辨这一点的关键手段
2. **`mouse.swift` 的 click 从来不设 `kCGMouseEventClickState`**，WebKit 不认作点击——而 hover 是好的，所以现象是「能悬停不能点」。旧工具链一直有这个洞
3. **CGEvent 的 modifier flag 会泄漏到后续鼠标事件**：发完 `⌃⇧P` 再点击 = ctrl+click = 右键菜单，而原生菜单会吞掉全部输入，导致后续步骤静默失效。修法是在鼠标事件上显式 `e.flags = []`
4. **`screencapture -l` 会返回隐藏窗口的陈旧 backing store**——看起来是一张正常的实时截图，实际是上一次可见时的画面。**每次截图前后都要用窗口列表确认在屏**，否则会基于幻觉做判断

### 未纳入（范围外，显式留痕）

- **`user_version` 11 → 12** 是本次唯一不可逆项。MCP 进程共享同库，**发版须同批**，否则旧二进制撞 `SchemaVersionMismatch`
- [[026-fixed-spatial-layout]] 的 features 节奏表缺行仍未补（第二段已留痕，本段沿用不夹带）
- [[06-prd]] `status: pre-code` 僵尸字段未动——它已与「文档描述的表全部落地」矛盾，但改 status 是治理信号，归 [[HANDOFF]] 旧账

## 2026-08-20（四）· 第二段 — ADR-025 契约回流八步

### 变更内容

把 [[025-unified-anchored-editing]] 的 P0 + P1-a + P1-b 落地事实回流到设计文档层（方法论 §7 八步）。**七笔契约，其中两笔是待办清单里漏记的**——[[HANDOFF]] 记了五笔，逐文件核实后另有两笔：子决策 2 的**保存语义规则表**本身，以及**容器形态**本身。清单从别处抄来时会漏，这与 P1-b「五个编辑面其实是四个」是同一类错误。

| 文档 | bump | 改动 |
|---|---|---|
| [[03-product-spec]] | v0.18 → **v0.19** | §13.3 新增**「编辑容器统一契约」跨区域小节**（容器形态 / 保存语义规则表五行 / Scene 属性面板点外拒绝关闭的唯一例外 / 提交键统一 A1-08）；区域 2-bis、3、4、草稿卡四处编辑描述改写；§13.4 补 `⌘⏎` 与编辑器内 `ESC` 两行 |
| [[05-design-spec]] | v0.17 → **v0.18** | 新增 **§2.6 层叠标尺**（`--z-*` 五档，补的是一个长期空白——此前 design-spec 从未记载任何 z 契约）；§10.2.2 新增 `AnchoredEditor` primitive + **接口契约小节**；§10.3 组件清单加行 |
| [[07-features]] | v1.13 → **v1.14** | §3.8 新增「统一锚定编辑容器」行 → `done`（P0+P1-a+P1-b）；§4 节奏表加行，合计 86 → **87**；§6 变更日志追加条目 |
| [[11-test-spec]] | v0.2 → **v0.3** | **全量刷新 📊 口径**（见下）+ 新增 §4.1 真机验收门 / §3.5 `density-gate` / §3.6 `theme-parity` / §7.3 jsdom `popover` shim |

### 回流时发现的三处账实不符

1. **`⌘Enter` 保存并推进到下一条从未实现**。子决策 2 写了该规则，但 `PhraseFormEditor.tsx:196-206` 只做保存并关闭——推进属 P2 连续处理模型。**已在 product-spec 显式标注「P2 目标，未落地」，不写成现状**。这正是本轮反复援引的那条纪律：规则表写了不等于实现了
2. **test-spec 的口径差了一个半月和两倍用例量**：前端 154 → **373**（17 → 37 文件）、Rust 135 → **158**、IPC 命令 48 → **51**、源码级 gate **4 → 6**（`density-gate` 与 `theme-parity` 早已落地并在 CI 跑，规格文件从未登记）。数字标了日期不算说谎，但已无参考价值，故全量重测重列
3. **`⌥ Space` 的「可配置」是自 v0.5 起写入契约却从未实现的欠账**（product-spec §13.4），实现硬编码于 `src-tauri/src/lib.rs:165`。本轮不改（属新立的 ADR-027 范围），但已在 v0.19 修订记录标注——**它说明「快捷键可配置」不是新想法，是旧承诺**

### 未纳入（范围外，显式留痕）

- **[[026-fixed-spatial-layout]] 缺 features §4 节奏表行与 §6 变更日志条目**（v1.12/v1.13 只写进了 §3.8 表）。属旧账八步范围，omar 已定本轮只做 ADR-025 涟漪、不夹带，故**只在 §4 表下留警示不补**。补账时须同步复核合计数
- design-spec §3c compact 层立论重写、`03-product-spec` §13.4 的 SOP instrument card 描述与单行 stub 实现不符——同属旧账

---

## 2026-08-20（四）· 第一段 — ADR-025 P1-b — 其余编辑面迁入锚定层
