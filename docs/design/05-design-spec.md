...
### 2.7 密度层（Density tier · v0.19 新增 — omar 2026-08-20 裁「留」）

> **本节又是一个长期空白**（同 §2.6 z 标尺）：紧凑档自 v0.13 起在 `tokens.css` §3c 落地并由 `density-gate.test.ts` 守着，**design-spec 从未记载过它**。缺席的后果不是没人用，是**没人能对它提出异议**——它唯一写下的存在理由还是错的。

**两档**：`舒适`（bare root，默认）/ `紧凑`（`:root.compact`，用户在设置 › 外观显式选择）。

#### 立论（v0.19 重写，旧理由已证伪）

旧文写「640px 基准窗口的高度大半被结构 chrome 吃掉」。**该基准不存在**：主形态窗口每次唤起都重新铺满当前显示器（`fit_to_active_monitor`），`tauri.conf.json` 的 `800×600` 只是占位。**一个用不可复现的数字辩护的档位，等于没人能反驳它**。

真实立论：

- **变量不是窗口，是显示器**。窗口恒等于屏高，所以约束来自**用户可能用的最小屏**
- **结构 chrome 近似恒定**（任务区四周约 `263px`，2026-08-19 实测），屏幕越小，它占的比例越大
- **在 1366×768 上**：任务区约 `505px`，而两条硬下限合计 `421px` —— **余量 84px；两条常驻横幅同时出现时只剩约 18px**
- 紧凑档买回的正是这段余量
...
| 规则 | 内容 |
|---|---|
| **只收结构，不动字号** | 密度不得以可读性为代价——字号 token 在两档间**逐字节相同** |
| **只能更紧** | 紧凑档的每个 override 必须重声明一个上文已定义的 token 且**取严格更小的 px 值**，由 `density-gate.test.ts` 卡死 |
| **舒适档 = 验收基线** | bare root 与 reshape-v2 验收基线**逐像素一致**，紧凑档是纯增量 |
| **一种密度声音** | 区域水平内边距等 token 也会波及设置弹窗 / 搜索覆盖层，**这是有意的**——不做「主区紧凑、附属面不紧凑」的分裂 |

**收益（由 `tokens.css` 数值计算，非屏幕实测）**：任务区外框竖向省 `20px`（相位带 8 + 对齐话术 8 + 状态栏 4），其内每个区域头再省 `8px`；**收益主要来自重复行单元**——Macro 磁贴 `56→44`、Scene 行 `32→28`，行数越多累积越多。
...
| ❌ **用文字标签向用户解释界面自身的层级架构**（v0.19 新增 · omar 2026-08-20）| 与上一条同源：层级已由**位置 + 形状**冗余编码，再挂一枚「协议层」「任务层」文字 pill 就是界面在讲解自己的架构，属解释性 UI。**已删除两枚**（`ProtocolBand` / `Dashboard` 任务列）。⚠️ **唯一例外：`ModifierGrid` 的「协议层 · 参考」pill 保留**——该面在 aside 列，没有暗色 band、也没有任务列的位置线索，那枚 pill 是它**唯一**的层级标识，删了会真丢信息，且它正好坐在任务侧的列里，最容易被误读为任务层资产 |
...
- **新增 §2.7 密度层**。紧凑档自 v0.13 起就在 `tokens.css` §3c 落地、由 `density-gate.test.ts` 守着，**design-spec 从未记载过它**——与 v0.18 补的 z 标尺是同一类空白。缺席的后果不是没人用，是**没人能对它提出异议**
- **立论换成可复现的**：变量不是窗口是**显示器**；结构 chrome 近似恒定（任务区四周约 `263px`），屏越小占比越大；1366×768 上任务区约 `505px` vs 硬下限合计 `421px`，余量 84px，两条常驻横幅同时出现时只剩约 18px
- **四条契约写进表**：只收结构不动字号 / 只能更紧（gate 卡死）/ 舒适档 = 验收基线逐像素一致 / 一种密度声音（附属面一并收紧，有意为之）
- **收益标明来源**：由 token 数值**计算**得出而非屏幕实测——任务区外框竖向省 20px，区域头各省 8px，主要收益来自重复行单元（Macro 磁贴 56→44 / Scene 行 32→28）
- 同步 `src/styles/tokens.css` §3c 与 `src/stores/settingsStore.ts` 的 `Density` 注释（两处都写着同一个不存在的基准）
- **§5 反设计清单新增一条：不用文字标签解释界面自身的层级架构**（omar 同日裁决）。删掉 `ProtocolBand`「协议层」与任务列「任务层」两枚 pill——层级已由位置 + 形状冗余编码（§2.4.1 / §5 既有条目），文字属重复标注。**保留 `ModifierGrid` 的「协议层 · 参考」**：该面在 aside 列，无 band 无位置线索，那枚 pill 是唯一标识。**三枚不是一组对称装饰，其中一枚承重**——这与 [[026-fixed-spatial-layout]] 裁决表第 3 项「承重件必须显式标注」是同一条纪律
