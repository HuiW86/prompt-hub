...
related:
  ...
  - m0-4-macos-signing
description: v0.1.0 首次打 tag 产出「签名但未公证」的包并绿灯进 draft。直接原因是 Tauri 凭据变量名不匹配导致公证被静默跳过；深层原因是 M0-4 验证过的「能力」在无人察觉中升格成了「该能力在 CI 上确实被行使」。记录 fail-open 断言这一类缺陷的四个同形实例与自查方式（第四例 2026-08-10 复发于第三方 action，见 §10）。
---
...
本次事故与 24 小时内另两处缺陷是同一形状，**都是断言写在命名或结构里、而不是写成可执行检查，且失败时 fail-open**。第四例在本文写成五天后复发，见 §10：
...
| `actions/download-artifact`（2026-08-10，§10） | 第三方 action 的语义 | 产物已过期时下载到 0 个文件，打印 `finished successfully` 并返回 0 |
...
7. **这一步是别人写的吗？** 前六问天然只会被用在自己写的检查上。第三方 action、CLI、SDK 同样会 fail-open，而它们的「成功」语义不由你定义——凡是下游依赖其副作用的，副作用要由**你自己**断言一次（§10）
...
## 10. 补遗：同类第四例（2026-08-10）

本文写成五天后，同一条流水线上出现第四个 fail-open。记在这里而不是另起一篇——它没有新的因果链，只有一个新的**盲区位置**。

**经过**：run `31147829395` 的 build 双架构全绿（公证闸门这次真的生效了），产物上传。sign job 停在 `release-signing` 人工审批上等了 3 天。审批放行后，`actions/download-artifact` 打印 `Found 0 artifact(s)` / `Total of 0 artifact(s) downloaded` / `Download artifact has finished successfully`，退出 0；下一步 `cp` 撞上空目录，报 `No such file or directory` 而死。

**直接原因**：upload 步骤写着 `retention-days: 1`，而它下游是一道**没有时间上界**的人工闸门。产物在 build 完成 24 小时后被 GitHub 回收，审批发生在第 72 小时。

**为什么 §8 清单没拦住它**：

- 第 3 问「零输入时它会怎样？」正好描述了这个形态，但清单被隐式地只用在**我们自己写的检查**上。`download-artifact` 是一行 `uses:`，读起来像基础设施而不像检查，于是没进入审视范围。这正是新增第 7 问的理由
- 第 2 问「失败路径被执行过吗？」在这里做不到——失败条件是「保留期 < 审批耗时」，不是构造一次输入就能触发的，它需要时间流逝。**跨越时间的耦合无法用一次快速测试证伪**，只能靠读配置时把两个数字放在一起看

**修复**：`retention-days` 1 → 7（覆盖真实审批节奏，且这批产物已公证但未 minisign，滞留不构成冒充更新的风险）；sign job 新增 `Assert build artifacts present`，对 4 个预期文件逐个点名，缺失时直接给出「多半是等审批期间过了保留期」的处置建议。

**这次的教训不是「再加一个闸门」，而是**：闸门本身也有依赖——`Assert notarized + stapled` 保证了产物合格，却管不着产物还在不在。**一条流水线的正确性不止取决于每步做对了什么，还取决于步与步之间的东西是否还活着。**
